...
<form method="post" name="DF" action="<?php eo($self) ?>" enctype="multipart/form-data">
    ...
    <div class="inv">
        ...
        <?php if ($_SESSION['is_logged'] && $dbh) { ?>
            | <a href="?<?php eo($xurl) ?>&q=show+databases">Databases</a>: <select name="db" onChange="frefresh()">
        ...
        <?php } ?>
        <?php if ($GLOBALS['ACCESS_PWD']) { ?> | <a href="?<?php eo($xurl) ?>&logoff=1"
                                                    onclick="logoff()">Logoff</a> <?php } ?>
    ...
    function print_screen()
    {
        ...
        <div class="dot" style="padding:0 0 5px 20px">
            SQL-query (or multiple queries separated by ";"):&nbsp;
            <button type="button" class="qnav" onclick="q_prev()">&lt;</button>
            <button type="button" class="qnav" onclick="q_next()">&gt;</button>
            <br>
...
function print_cfg()
{
    ...
    <center>
        ...
        <div class="frm">
            <label class="l">DB user name:</label><input type="text" name="v[user]"
                                                         value="<?php eo($DB['user']) ?>"><br>
            ...
            <div id="cfg-adv" style="display:none;">
                ...
                <label class="l">MySQL host:</label><input type="text" name="v[host]" value="<?php eo($DB['host']) ?>">
                port: <input type="text" name="v[port]" value="<?php eo($DB['port']) ?>" size="4"><br>
                <label class="l">Charset:</label><select name="v[chset]">
                    <option value="">- default -</option><?php echo chset_select($DB['chset']) ?></select><br>
    ...
    <center>
        ...
        <div class="frm">
            ...
            <div>
                ...
                <?php } else { ?>
                    <label>&nbsp;( ) .csv</label>
                    <small>(to export as csv - go to 'show tables' and export just ONE table)</small>
                <?php } ?>
            </div>
            <br>
            ...
            <input type="submit" value=" Download "><input type="button" value=" Cancel "
                                                           onclick="window.location='<?php eo($self . '?' . $xurl . '&db=' . $DB['db']) ?>'">
        </div>
    </center>
    <?php
    print_footer();
...
function print_import()
{
    ...
    <center>
        ...
        <div class="frm">
            ...
            <input type="submit" value=" Upload and Import " onclick="return ays()"><input type="button"
                                                                                           value=" Cancel "
                                                                                           onclick="window.location='<?php eo($self . '?' . $xurl . '&db=' . $DB['db']) ?>'">
        </div>
        <br><br><br>
